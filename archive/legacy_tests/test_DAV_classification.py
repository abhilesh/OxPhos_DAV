"""
4-Layer cDAV/uDAV Classification Verification Diagnostic
=========================================================
...
for BOTH mtDNA and nucDNA datasets:
  ...
  Layer 2 — Species Claim Validation : direct alignment re-scan for each cDAV call
...
def layer1_triangle_of_truth(variants: list, genome: str, cache: dict) -> bool:
    ...
    print(f"{N_TRIALS} trials × {SAMPLE_SIZE} random cDAVs per trial.\n")

    eligible = _build_layer1_eligible(variants, genome, cache)
    if not eligible:
        print("  No eligible cDAVs found — skipping.")
        return True
